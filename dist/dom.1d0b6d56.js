...
window.dom = {
  create: function create(string) {
    var container = document.createElement('template'); //因为template可以容纳所有元素，例如div就不能容纳td标签
  ...
  empty: function empty(node) {
    //也可以直接将innerHtml设置为空，但不好将移去的内容返回
  ...
  text: function text(node, string) {
    //如果只有一个参数就读取内容，两个参数就修改内容，函数重载
    if (arguments.length === 1) {
      if ('innerText' in node) {
        //防止ie浏览器没有innerText属性
  ...
  style: function style(node, name, value) {
    if (arguments.length === 3) {
      //(node,'width','40px')形式的修改
      node.style[name] = value;
    } else if (arguments.length === 2) {
      if (typeof name === 'string') {
        return node.style[name]; //读取某个节点某个样式的属性值
      } else if (name instanceof Object) {
        //(node,'width:300px,height:100px')形式的修改
        for (var key in name) {
          //要在key前面加上let
  ...
  on: function on(node, event, f) {
    node.addEventListener(event, f);
  },
  off: function off(node, event, f) {
  ...
  siblings: function siblings(node) {
    return Array.from(node.parentNode.children).filter(function (value) {
      return value !== node;
    });
  },
  ...
  previous: function previous(node) {
    var x = node.previousSibling; //没有previousElementSibling属性
...
},{}]},{},["C:/Users/小仙女/AppData/Local/Yarn/Data/global/node_modules/parcel/src/builtins/hmr-runtime.js","dom.js"], null)
